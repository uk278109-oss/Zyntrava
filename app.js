...
async function api(action,body={}){if(!Z.user)throw new Error('Please sign in again.');const token=await Z.user.getIdToken(true);let r;try{r=await fetch('/api?action='+encodeURIComponent(action),{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+token},body:JSON.stringify(body),cache:'no-store'});}catch(e){throw new Error('Zyntrava API is unreachable. Make sure this project is deployed on Vercel with the /api folder at the project root.')}const text=await r.text();let d;try{d=JSON.parse(text)}catch(e){throw new Error('API returned a non-JSON response ('+r.status+'). Check that /api/index.js is deployed at the project root.')}if(!r.ok||d.ok===false)throw new Error(d.error||('API request failed ('+r.status+')'));return d}
...
async function loadAccountSummary(){
  try{
    ...
    const box=document.querySelector('[data-api-error]'); if(box) box.textContent='Backend: '+e.message;
    return null;
  }
}
