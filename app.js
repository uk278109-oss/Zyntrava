...
async function spinReward(){try{const d=await api('spin');sound('coin');toast(`Spin complete: +${d.reward} ZN.`);const a=await api('account-summary');syncAccountUI(a);if(document.body.dataset.page==='dashboard')loadDashboard()}catch(e){toast(e.message)}}
...
function syncAccountUI(d){
  if(!d)return;
  if(Number.isFinite(Number(d.points))) Z.profile={...(Z.profile||{}),points:Number(d.points)};
  if(Number.isFinite(Number(d.earnedPoints))) Z.profile={...(Z.profile||{}),earnedPoints:Number(d.earnedPoints)};
  if(Number.isFinite(Number(d.pointsLocked))) Z.profile={...(Z.profile||{}),pointsLocked:Number(d.pointsLocked)};
...
async function loadDashboard(){
  ...
  try{
    ...
    if(activity){
      ...
      activity.innerHTML=tx.length?tx.slice(0,5).map(x=>`<div class="row"><div><b>${esc(x.description||x.type||'Activity')}</b><p class="muted">${esc(x.status||'completed')}</p></div><b class="${x.direction==='debit'?'muted':'reward'}">${x.direction==='debit'?'-':'+'}${fmt(x.amount)} ZN</b></div>`).join(''):'<div class="empty">No verified account activity yet.</div>';
    }
  }catch(e){
    console.error('Dashboard summary:',e);
    if(activity) activity.innerHTML='<div class="empty"><b>Account activity is temporarily unavailable.</b><br><span class="muted">Your wallet balance is loaded from the live account record. Please refresh in a moment.</span></div>';
  }
}
