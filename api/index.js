...
async function handleCpxSurveys(req, res) {
  ...
  // CPX requires the real user's IP for region matching and fraud detection.
  ...
  catch (_) { throw Object.assign(new Error('CPX returned an invalid response'), { status: 502 }); }

  if (!response.ok || data.status !== 'success') {
    const message = String(data.message || data.error || 'CPX surveys could not be loaded');
