...
async function auth(req) {
  ...
  return await withTimeout(admin.auth().verifyIdToken(h.slice(7)), 15000, 'Authentication service timed out. Please try again.');
}
...
// Prevent any external/Firebase operation from keeping a Vercel request alive for minutes.
...
async function fetchWithTimeout(url, options = {}, ms = 30000) {
  ...
  try {
    ...
    if (e && e.name === 'AbortError') {
      throw Object.assign(new Error('External service timed out. Please try again in a moment.'), { status: 504 });
    }
    throw e;
  } finally {
    clearTimeout(timer);
...
async function reconcileUserFromLedger(uid) {
  const [userSnap, txSnap, notifSnap, dailySnap] = await Promise.all([
    db().ref('users/' + uid).once('value'),
    db().ref('transactions/' + uid).once('value'),
    db().ref('notifications/' + uid).once('value'),
    db().ref('dailyClaims/' + uid).once('value')
  ]);
  ...
  for (const tx of txs) {
    if (tx?.id) transactionKeys.add(String(tx.id));
  ...
  // Some older claims successfully marked users/{uid}/lastDailyClaim and
  // dailyClaims/{uid}/{date}, but failed before writing the transaction ledger.
  // Recover those backed-by-record claims exactly once.
  let unledgeredDailyCredits = 0;
  for (const [date, claim] of Object.entries(dailySnap.val() || {})) {
    const amount = Number(claim?.rewardZN || 0);
    if (!Number.isFinite(amount) || amount <= 0) continue;
    const ledgerId = 'daily_' + date;
    if (!transactionKeys.has(ledgerId)) unledgeredDailyCredits += amount;
  }
  const missingDailyCredits = Math.max(0, notificationDailyCredits - ledgerDailyCredits);
  const recoveryCredits = Math.max(missingDailyCredits, unledgeredDailyCredits);

  const currentPoints = Number(user.points || 0);
  const currentEarned = Number(user.earnedPoints || 0);
  const ledgerNet = Math.max(0, Math.round(ledgerCredits - ledgerDebits));
  const expectedPoints = Math.max(currentPoints, ledgerNet + Math.round(recoveryCredits));
  const expectedEarned = Math.max(currentEarned, Math.round(ledgerCredits + recoveryCredits));
  ...
  for (const [date, claim] of Object.entries(dailySnap.val() || {})) {
    const amount = Number(claim?.rewardZN || 0);
    const ledgerId = 'daily_' + date;
    if (Number.isFinite(amount) && amount > 0 && !transactionKeys.has(ledgerId)) {
      await db().ref('transactions/' + uid + '/' + ledgerId).set({
        id: ledgerId, type: 'daily', amount, direction: 'credit', status: 'completed',
        description: 'Daily reward (recovered)', rewardDate: date, createdAt: claim?.createdAt || Date.now(), recovered: true
      });
    }
  }
...
async function handleCpxPostback(req, res) {
  ...
  // CPX postbacks must remain authenticated. Never accept a conversion
  // without the publisher secret configured.
  if (!expectedSecret) throw Object.assign(new Error('CPX_SECURE_HASH is not configured in Vercel Environment Variables'), { status: 503 });
...
async function handleCpxSurveys(req, res) {
  ...
  const appId = String(process.env.CPX_APP_ID || '35839').trim();

  const crypto = require('crypto');
  const secureHash = secret ? crypto.createHash('md5').update(`${u.uid}-${secret}`).digest('hex') : '';
  ...
  const params = new URLSearchParams({
    app_id: appId, ext_user_id: String(u.uid), email: String(u.email || ''),
    subid_1: '', subid_2: '', output_method: 'api', ip_user: ip,
    user_agent: userAgent, limit: '12', ...(secureHash ? { secure_hash: secureHash } : {}),
    main_info: 'true', user_country_code: String(profile.countryCode || 'PK')
  });
  const url = `https://live-api.cpx-research.com/api/get-surveys.php?${params.toString()}`;
  let response;
  try { response = await fetchWithTimeout(url, { method: 'GET', headers: { 'Accept': 'application/json' } }, 30000); }
  ...
  return send(res, 200, {
    ...
    available: Number(data.count_available_surveys || 0),
    ...
    surveyWallUrl: `https://wall.cpx-research.com/index.php?app_id=${encodeURIComponent(appId)}&ext_user_id=${encodeURIComponent(u.uid)}`
  });
}
...
async 'cpx-surveys'(req, res) { return await handleCpxSurveys(req, res); },
  async 'cpx-frame'(req, res) {
    ...
    return send(res, 200, { ok: true, url: `https://wall.cpx-research.com/index.php?${params.toString()}` });
  },
  async 'health'(req, res) {
    const checks = {
      ...
      adminUids: Boolean(process.env.ADMIN_UIDS),
      dailyReward: Boolean(process.env.DAILY_REWARD_POINTS || process.env.DAILY_REWARD),
      spinRewards: Boolean(process.env.SPIN_REWARDS)
    };
    return send(res, 200, { ok: true, service: 'zyntrava-api', checks, ready: checks.firebaseProject && checks.firebaseClientEmail && checks.firebasePrivateKey && checks.firebaseDatabaseUrl });
  },
  ...
  async 'bootstrap-user'(req, res) {
    ...
    let [userSnap, claimSnap, ledgerSnap] =
      await Promise.all([
        userRef.once('value'),
        claimRef.once('value'),
    ...
    if (!userSnap.exists()) {
      ...
      await userRef.set({
        ...
        email: u.email || '',
        displayName,
        memberId: 'ZY-' + u.uid.slice(0, 6).toUpperCase(),
        referralCode: 'ZY' + u.uid.slice(0, 8).toUpperCase(),
        countryCode: 'PK',
        points: 0,
        pointsLocked: 0,
        earnedPoints: 0,
  ...
  async 'spin'(req, res) {
    ...
    const rewards = String(process.env.SPIN_REWARDS || '2,5,10,15')
      .split(',')
      .map(x => Math.round(Number(x.trim())))
      .filter(x => Number.isFinite(x) && x > 0);

    if (!rewards.length) {
      throw Object.assign(new Error('Spin reward configuration is invalid'), { status: 503 });
    }

    const userRef = db().ref('users/' + u.uid);
    let before = await userRef.once('value');
    ...
    if (!before.exists()) {
      ...
      before = await userRef.once('value');
    }

    const ledgerRef = db().ref('transactions/' + u.uid + '/spin_' + key);

    // Repair an older successful spin whose ledger write was interrupted.
    const existing = before.val() || {};
    if (String(existing.lastSpin || '') === key) {
      const reward = Number(existing.lastSpinRewardZN || 0);
      if (reward > 0 && !(await ledgerRef.once('value')).exists()) {
        await withTimeout(ledgerRef.set({
          id: ledgerRef.key,
          type: 'spin',
          amount: reward,
          direction: 'credit',
          status: 'completed',
          description: 'Daily promotional spin (recovered)',
          rewardDate: key,
          createdAt: existing.lastSpinAt || Date.now(),
          recovered: true
        }), 5000);
      }
      return send(res, 200, {
        ...
        earnedPoints: Number(existing.earnedPoints || 0),
        pointsLocked: Number(existing.pointsLocked || 0),
        lastSpin: key,
        saved: true
      });
    }
    ...
    const tx = await userRef.transaction(profile => {
      if (!profile || String(profile.lastSpin || '') === key) return;
      claimed = true;
      return {
        ...profile,
        points: Number(profile.points || 0) + reward,
        earnedPoints: Number(profile.earnedPoints || 0) + reward,
        lastSpin: key,
        lastSpinRewardZN: reward,
        lastSpinAt: admin.database.ServerValue.TIMESTAMP
      };
    });

    const current = tx.snapshot?.exists?.()
      ? (tx.snapshot.val() || {})
      : ((await userRef.once('value')).val() || {});
    ...
    if (!tx.committed || !claimed) {
      return send(res, 409, {
        ok: false,
        error: 'Spin could not be saved. Please try again.',
        points: Number(current.points || 0),
        earnedPoints: Number(current.earnedPoints || 0),
        pointsLocked: Number(current.pointsLocked || 0)
      });
    }

    // The wallet transaction above is authoritative. Ledger/notification are
    // secondary records and can be repaired safely on a later account-summary.
    try {
      await withTimeout(ledgerRef.set({
        id: ledgerRef.key,
        type: 'spin',
        ...
        direction: 'credit',
        status: 'completed',
        description: 'Daily promotional spin',
        rewardDate: key,
        createdAt: admin.database.ServerValue.TIMESTAMP
      }), 5000);
    } catch (_) {}
    ...
    try {
      await withTimeout(db().ref('publicProfiles/' + u.uid).update({
        displayName: current.displayName || 'Member',
        countryCode: current.countryCode || 'Global',
        earnedPoints: Number(current.earnedPoints || 0)
      }), 5000);
    } catch (_) {}

    try {
      await withTimeout(addNotification(u.uid, 'Spin reward', `You received +${reward} ZN.`, 'reward'), 5000);
    } catch (_) {}
...
module.exports = async (req, res) => {
  ...
  const handler = handlers[action];
    ...
    return await withTimeout(handler(req, res), 25000, 'API request timed out. The backend or external service did not respond within 25 seconds.');
