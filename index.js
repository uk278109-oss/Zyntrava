...
function getCachedSummary(uid) {
  const item = summaryCache.get(uid);
  if (item && Date.now() - item.time < 8000) return item.data;
  return null;
}
function setCachedSummary(uid, data) {
  summaryCache.set(uid, { time: Date.now(), data });
  if (summaryCache.size > 500) summaryCache.clear();
}
function clearCachedSummary(uid) { summaryCache.delete(uid); }
...
const handlers = {
  ...
  async 'account-summary'(req, res) {
    const u = await auth(req);
    const cached = getCachedSummary(u.uid);
    if (cached) return send(res, 200, { ok: true, ...cached });
