...
async 'cpx-surveys'(req, res) { return await handleCpxSurveys(req, res); },
  ...
  async 'spin'(req, res) {
    ...
    // Make sure a user record exists before the transaction. Older versions
    // treated a missing users/{uid} record as "already spun", so the wheel
    // could animate while no balance was ever created.
    const userRef = db().ref('users/' + u.uid);
    const before = await userRef.once('value');
    ...
    // Choose the reward once per request. The database transaction is the
    // source of truth and prevents a second successful spin on the same day.
    const reward = rewards[Math.floor(Math.random() * rewards.length)];
    let claimed = false;

    const tx = await userRef.transaction(profile => {
      if (!profile) return;
      if (String(profile.lastSpin || '') === key) return;

      claimed = true;
      const nextPoints = Number(profile.points || 0) + reward;
      const nextEarned = Number(profile.earnedPoints || 0) + reward;

      return {
        ...profile,
        points: nextPoints,
        earnedPoints: nextEarned,
        lastSpin: key,
    ...
    const current = tx.snapshot && tx.snapshot.exists()
      ? (tx.snapshot.val() || {})
      : ((await userRef.once('value')).val() || {});
    ...
    if (!tx.committed || !claimed) {
      return send(res, 200, {
        ok: true,
        success: false,
        alreadySpun: String(current.lastSpin || '') === key,
        reward: 0,
        error: String(current.lastSpin || '') === key
          ? 'You already used today’s spin.'
          : 'Spin could not be saved. Please try again.',
        points: Number(current.points || 0),
        earnedPoints: Number(current.earnedPoints || 0),
        pointsLocked: Number(current.pointsLocked || 0),
        lastSpin: current.lastSpin || null
      });
    }

    // Deterministic ledger key: retrying the request can never create a
    // second ledger entry for the same daily spin.
    ...
    try {
      await withTimeout(
        addNotification(u.uid, 'Spin reward', `You received +${reward} ZN.`, 'reward'),
        5000
      );
    } catch (_) {}
