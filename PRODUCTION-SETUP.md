...
## SINGLE API ROUTER (VERCEL HOBBY)
This project intentionally uses one serverless function: `api/index.js`. All frontend requests must use `/api?action=<action>`; do not add separate files under `api/` unless the Vercel function limit is considered.
...
Optional: DAILY_REWARD_POINTS, SPIN_REWARDS, POINTS_PER_CURRENCY_UNIT
